...
print("📥 [CLIENTE 2] Cargando dataset...")
...
print(f"🚀 [CLIENTE 2] Iniciado. Enviando tweets...")

try:
    for index, row in df.iloc[1::2].iterrows(): # Toma las filas impares
        comment_text = str(row[columna_texto])
        if not comment_text or comment_text.strip() == "" or comment_text == "nan":
            continue

        mensaje = {
            "user_id": f"user_c2_{random.randint(500, 999)}",
        ...
        print(f"🔵 [CLIENTE 2] Fila {index} enviada (Espera: {delay}s) | {comment_text[:40]}...")
        time.sleep(delay)
